...
# Kieser M. and Friede T., 2003. Simple procedures for blinded sample size adjustment that
# do not affect the type I error rate. Statistics in Medicine. 2003 Dec 15;22(23): 3571-81.
...
for(iter in 1:rows)
{
  ...
  # sigma = 6 # Assumed parameters for us to plug
  sigma = iter_sigma[iter]
  Delta = 2.34 # into the sample size calculation.
  
  alpha = 0.05 # Desired error rates for our
  beta = 0.2 # hypothetical trial.
  ...
  Nsim = 500000 # Number of simulated trials to run.
  
  
  pvalues = rep(NA, Nsim) # An empty vector to store the p-values.
  sigma_adj_reestimates = rep(NA, Nsim) # An empty vector to store the p-values.
  ...
  for(i in 1:Nsim){
    ...
    # n1 is the interim number of samples per group
    interim = round(n/2, 0)
    ...
    # sigma adjusted
    sigma_adj =   sqrt(((2*(interim - 1)*(sigma_reestimate^2))/(2*interim - 1)) + 
                         ((interim*((mean(resultsC) - mean(resultsT))^2))/(2*(2*interim - 1))) -
                         ((interim * (Delta^2))/(2*(2*interim - 1))))
    
    sigma_adj_reestimates[i] = sigma_adj
    
    # N rescaled
    n_reestimate = round(2*(sigma_adj^2)*((qnorm(1 - alpha) + qnorm(1 - beta))^2)/(Delta^2))
    n_reestimate
    N_reestimates[i] = 2*n_reestimate
    
    if(sigma_adj > sigma)   # We under-estimated sigma at the start (SSR will increase ss)
    {
    ...
    if(sigma_adj < sigma)   # We over-estimated sigma at the start (SSR will increase ss)
    {
  ...
  sigma_adj_reestimate = mean(sigma_adj_reestimates)
  cat("\nSigma Adjusted re-estimate: ", sigma_adj_reestimate)
  
  
  cat("Final re-estimated sample size: ", round(mean(N_reestimates)))
  
  
  iter_sigma_reestimate[iter] = sigma_adj_reestimate
...
results4_1 = data.frame("delta" = rep(0,10),
...
for(i in 1:10){
  ...
  print(ss_CI)
  ss_reestimate[i] = paste(results4_1$Sample.Size.Re.estimate[i], " (" , round(ss_CI[1],3), ",",  round(ss_CI[2],3), ")", sep = "")
}

results4_1$Sample.Size.Re.estimate = ss_reestimate

write.csv(results4_1, "Results/met4_blinded_h0.csv", row.names = FALSE)


write.csv(df_iter_ss_reestimate, "Results/met4_blinded_h0_distribution.csv", row.names = FALSE)
...
for(iter in 1:rows)
{
  ...
  pvalues = rep(NA, Nsim) # An empty vector to store the p-values.
  sigma_adj_reestimates = rep(NA, Nsim) # An empty vector to store the re-estimated sigma values.
  N_reestimates = rep(NA, Nsim)
  ...
  sigma_reestimate = mean(sigma_adj_reestimates)
...
results4_2 = data.frame("delta" = rep(2.34,10),
                        "Sigma TRUE" = iter_sigmaTRUE, "Sigma Assumed" = iter_sigma,
                        "Sigma Re-estimate" = iter_sigma_reestimate, 
                        "Sample Size Initial" = non_ssr_ss, "Sample Size Re-estimate" = iter_ss_reestimate, 
...
for(i in 1:10){
  ...
  ss_CI = c(round(mean(df_iter_ss_reestimate[,i]))- ss_std_error, 
            round(mean(df_iter_ss_reestimate[,i]))+ ss_std_error)
  print(ss_CI)
  ss_reestimate[i] = paste(results4_2$Sample.Size.Re.estimate[i], " (" , round(ss_CI[1],3), ",",  round(ss_CI[2],3), ")", sep = "")
}

results4_2$Sample.Size.Re.estimate = ss_reestimate


write.csv(results4_2, "Results/met4_blinded_h1.csv", row.names = FALSE)


write.csv(df_iter_ss_reestimate, "Results/met4_blinded_h1_distribution.csv", row.names = FALSE)
